...
| "Kani-proves the SHIPPED SPL/System CPI encoders" | ✅ the harnesses now call the shipped `hopper_runtime::{token,system}::encoders::*` directly (17 wire-format + 17 differential-oracle vs an independent reference), byte-identical to what the CPI emits. ⚠️ scope: the **fixed-size** instruction set. The System `*WithSeed` family splices a runtime seed slice and is NOT proven — never imply it is |
| "Smallest binary" (again) | ⚠️ the vault `.text` is **re-measured at 14.03 KiB**, not the 7.46 KiB still printed in `BENCHMARKS.md`. Batches 5–7 regressed it; `.bss`/`core::fmt` fixes clawed back ~37 KiB of `.so`. Quote nothing from that size row until it is re-run |
