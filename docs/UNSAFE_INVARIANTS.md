...
## Manual unsafe-source snapshot (non-exhaustive)

The table below is a legacy review snapshot retained for module-level context.
It is not a current or exhaustive count: active runtime work can add, remove,
or move unsafe boundaries. For every commit or release candidate, use the
artifact generated by `scripts/check-unsafe-safety-comments.py
--inventory-out unsafe-inventory.md`. That scanner checks the audited source
directories for non-rustdoc `unsafe {` blocks and `pub unsafe fn` declarations,
requires nearby `// SAFETY:` comments and rustdoc `# Safety` sections, and
records the exact revision's file/count inventory.
