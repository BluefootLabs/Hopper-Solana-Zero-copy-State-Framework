...
This ledger explains the major unsafe boundaries, their justifications, and
the invariants that must hold. The generated CI inventory, not the manually
maintained table below, is the exhaustive file/count record for a particular
source revision.
...
## Source-pinned unsafe inventory

This generated snapshot replaces the historical table, including removed
compatibility modules. Counts identify review surfaces, not a proof of soundness.
Regenerate for a new executable-source revision. The scan covers all 29 public
packages; the detailed explanations below focus on the account/runtime layers.

Generated by `scripts/check-unsafe-safety-comments.py`.

Publish-order SHA-256: `0fe599a7df87d80ce631370981e59ae1503cfe9f1455a34cd1db75affb440f55`
Source commit: `05afb389d2e4bcbbc6d6a21a16de8c9b5b0b2717`
Source tree clean: `true`
Public packages: 29
Checked Rust files: 310
...
| `crates/hopper-native/src/borrow.rs` | 5 | 2 | 2 | 0 | 0 | 0 |
| `crates/hopper-native/src/budget.rs` | 2 | 0 | 0 | 0 | 0 | 0 |
| `crates/hopper-native/src/cpi.rs` | 15 | 2 | 2 | 0 | 0 | 0 |
| `crates/hopper-native/src/entrypoint.rs` | 20 | 8 | 4 | 0 | 2 | 0 |
...
| `crates/hopper-runtime/src/account.rs` | 42 | 9 | 8 | 0 | 16 | 0 |
...
| `crates/hopper-runtime/src/borrow.rs` | 14 | 2 | 2 | 0 | 0 | 0 |
...
| `crates/hopper-runtime/src/cpi_dedup_tests.rs` | 2 | 0 | 0 | 0 | 0 | 0 |
...
| `crates/hopper-runtime/src/lib.rs` | 18 | 3 | 3 | 0 | 0 | 0 |
...
| Invariant | Focused suite | Command |
|---|---|---|
| Pod boundary rejection | `tests/unsafe_boundary_tests.rs` | `cargo test --test unsafe_boundary_tests` |
| Overlay checked/unchecked parity | `tests/overlay_equivalence_tests.rs` | `cargo test --test overlay_equivalence_tests` |
| Compat regression and receipt wire format | `tests/compat_regression_tests.rs` | `cargo test --test compat_regression_tests` |
| Property-based ABI roundtrip | `tests/property_tests.rs` | `cargo test --test property_tests` |
| CPI guard, collections, registry, validation | `tests/trust_tests.rs` | `cargo test --test trust_tests` |

The suites are the durable evidence; their case counts move as regressions are
added and are intentionally not frozen here.
...
| `hopper-solana::cpi_guard` | Instructions sysvar borrow | Immutable read for validation | `cpi_guard_*` trust tests covering all guard variants |
| `receipt` | Fingerprint hashing of account data | FNV-1a deterministic, before/after tracked | `receipt_*` trust tests + `prop_receipt_*` property tests |
...
## Internal Hopper safety-review response (2026-04)

An internal safety review flagged four specific unsound or permissive
surfaces. Earlier revisions incorrectly called that review an independent
"Hopper Safety Audit" and linked an absent `docs/Hopper Safety Audit.docx`.
No independent third-party audit was performed. This section records the
internal findings, the action taken on each, and the invariants the fixes now
enforce.
...
This section enumerates every item in the April 2026 internal safety review
...
| M5 | Compile-fail doctests for negative proof | `crates/hopper-runtime/src/pod.rs` carries 3 compile-fail doctests for non-Pod overlays; at the 2026-08-16 source snapshot the root `tests/ui.rs` suite has 17 compile-fail fixtures under `tests/compile_fail/`, while `tests/hopper-trybuild/tests/ui/` has 14 fail fixtures and 6 pass fixtures. Treat these as dated counts; the commands in D2 are the evidence. |
...
| DX3 | Authored-source output pipeline | **DONE for the shipped Rust-authoring path**. Source-derived manifest emission feeds IDL, Codama, schema, and client outputs through `hopper compile --emit`. This is not a claim that Hopper implements a separate authored programming language. |
...
| Typed-context validation binding (audit final-API Step 7) | `crates/hopper-macros-proc/src/program.rs::handler_invocation` emits `<ContextSpec>::bind(ctx)?` or `bind_with_args(...)` for typed `Ctx<Spec>` handlers before calling their body. Binding performs the checks declared by that context. Raw `&mut Context` handlers receive the raw context and own their validation, regardless of the `strict` intent flag. |
...
| Explicit token-authority pre-checks (`enforce_token_checks` is intent) | `crates/hopper-runtime/src/token.rs::require_token_authority` compares the token account's owner field at bytes `[32..64]` with the authority address; mismatch returns `IncorrectAuthority`. It does not establish token-program ownership, validate the complete layout, or check signer privilege. `TransferChecked`, `BurnChecked`, and `ApproveChecked` call it from `invoke_strict()` and `invoke_signed_strict()`. The direct strict methods also require signer privilege and return `MissingRequiredSignature` when absent. Signed strict methods pass PDA seeds for CPI runtime validation. The policy constant does not insert these calls. Owner-specific strict methods do not cover every valid delegate or multisig operation. The existing token-authority tests cover matching owner, mismatch, and short buffer cases. |
...
Post-CPI validation is deliberately split by handle type:

- `ForeignLens<T>` holds its account-data borrow for the lens lifetime, so every
  safe CPI tier refuses a writable hand-off until that lens is dropped.
- `ExternalAccount<T>` repeats the adapter's owner, discriminator, length, and
  custom validation before every safe data, view, resolve, explain, or checked-
  proof issuance path.
- `InterfaceAccount<T>` repeats owner-set and interface-layout validation before
  every typed load or resolution. A changed owner fails with
  `IncorrectProgramId` before bytes are interpreted.
- `Account<T>` and `InitAccount<T>` are entry/lifecycle-bound roles. Generated
  validation initially pins `Account<T>` to the executing program and later
...
- `SystemAccount` is also a point-in-time role and exposes no system-typed data.
  ...
  role again.
- `ExternalChecked<T, P>` revalidates the adapter's base owner/type contract on
  later view access, but proof-specific mint, authority, freshness, or other
  business facts are point-in-time evidence. They must not cross a CPI that can
  mutate the proven account; callers re-run `checked()` afterward. The unsafe
  unchecked CPI tier cannot enforce this rule.
...
cargo test --test ui --features proc-macros --locked
cargo test -p hopper-trybuild --locked
...
## September 26 lifecycle correction (0.4.2)

Host borrow tracking did not exercise the former unchecked SBF segment path.
Segment guards now retain the canonical native borrow as well as their range
lease. Compiled v0/v3 regressions test conflicts through a second registry,
whole-account access, resize, close, and checked CPI. Explicit disjoint splits
retain one whole-account exclusive borrow. Construction is crate-private so
unchecked offsets cannot reach the safe `all_mut` and `get_mut` methods.

Close operations preflight all applicable local checks before changing data or
balances. Native borrow mappings transfer the same lease; a panicking mapping
closure still drops the original guard during host unwinding. This targeted
review and the generated inventory are not an independent security audit.
