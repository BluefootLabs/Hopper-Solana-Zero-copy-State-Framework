...
| Cicada flagship custody and route safety | Internally hardened; 21 host and 22 strict compiled-SBF tests pass against canonical SPL Token, Token-2022, and hostile route fixtures, including exact wrapped-SOL lamport coupling on both canonical processors and atomic rollback of a modeled native-account shortfall. The post-closure isolated ELF reproduces byte-for-byte and passes the full binary-backed publish check. | Strong local evidence only. Retain the independent-audit, public-cluster/validator-replay, issuer-authority, upgradeable-route, narrow Token-2022, and dynamic downstream-attribution limits in every release claim. |
| Reproducible head-to-head benchmark | A required five-way Hopper/Pinocchio/Quasar/Anchor v2/Star Frame fixture now passes same-behavior checks and records source/binary provenance | The current run is diagnostic while source trees are dirty. A clean committed rerun is still required before replacing public numbers; blanket claims remain prohibited. |
...
| P1 | Clean same-fixture peer benchmark artifact | The required Hopper, Anchor v2, Quasar, Pinocchio, and Star Frame fixture is implemented and behavior-gated. Commit both source trees, rerun, and archive CU, binary size, build-time, hashes, and clean-tree provenance. |
