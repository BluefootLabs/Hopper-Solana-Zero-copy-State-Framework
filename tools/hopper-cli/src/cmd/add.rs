...
/// Inject a handler into the `#[hopper::program] mod app {
...
fn try_wire_dispatch(lib_rs: &Path, snake: &str, pascal: &str) -> Result<DispatchWiring, String> {
    ...
    if let Some(program_start) = content.find("#[hopper::program]") {
        ...
        let injected_handler = format!(
            "\n    #[instruction({next_disc})]\n    pub fn {snake}(ctx: Ctx<{pascal}>) -> ProgramResult {{\n        ctx.accounts.{snake}()\n    }}\n",
        );
...
mod tests {
    ...
    fn old_surface_terms() -> [&'static str; 4] {
        [
            concat!("#[hopper", "::context]"),
            concat!("Account", "View"),
            concat!("#[", "signer]"),
            concat!("ctx: ", "Context<"),
        ]
...
"#;
    ...
    #[test]
    fn instruction_template_uses_framework_first_surface() {
        let body = render_instruction_template("create_vault", "CreateVault");
        let task_marker = ["TO", "DO"].concat();

        for forbidden in old_surface_terms() {
            assert!(
                !body.contains(forbidden),
                "instruction template leaked old surface: {forbidden}"
            );
        }
        assert!(
            !body.contains(&task_marker),
            "instruction template leaked an unfinished-task marker"
        );
        ...
        for required in [
            ...
            "Signer<'info>",
            "Account<'info, Vault>",
            "pub fn create_vault(&self) -> ProgramResult",
        ] {
            assert!(
                body.contains(required),
                "instruction template missing hero surface: {required}"
            );
        }
    }
...
"#,
        ...
        assert!(lib.contains("pub fn transfer(ctx: Ctx<Transfer>) -> ProgramResult"));
        assert!(lib.contains("ctx.accounts.transfer()"));
        assert!(!lib.contains(concat!("Context", "<Transfer>")));
        assert!(!lib.contains(concat!("#[hopper", "::pipeline]")));
        assert!(!lib.contains(&task_marker));

        let ix = fs::read_to_string(dir.join("src/instructions/transfer.rs")).unwrap();
        assert!(ix.contains("#[derive(Accounts)]"));
        assert!(ix.contains("pub struct Transfer<'info>"));
        assert!(ix.contains("Signer<'info>"));
        assert!(ix.contains("pub fn transfer(&self) -> ProgramResult"));
        for forbidden in old_surface_terms() {
            assert!(
                !ix.contains(forbidden),
                "generated instruction leaked old surface: {forbidden}"
            );
        }
        assert!(!ix.contains(&task_marker));
