...
macro_rules! hopper_layout {
    (
        ...
        // Bytemuck proof (Hopper Safety Audit Must-Fix #5): every
        // field must itself satisfy `bytemuck::Pod + Zeroable`.
        // Hopper's Pod supertrait requires these impls; because all
        // field types generated by `hopper_layout!` are Hopper wire
        // types (which carry their own bytemuck impls) or byte
        // arrays, these blanket claims are safe.
