...
macro_rules! hopper_layout {
    (
        ...
        const _: fn() = || {
            fn __hopper_require_pod<T: $crate::hopper_core::account::Pod>() {}
            $(__hopper_require_pod::<$fty>();)+
        };
        ...
        impl $name {
            ...
            #[inline]
            pub fn load<'a>(
                account: &'a $crate::hopper_runtime::AccountView<'a>,
            ...
            pub fn verify_pda_cached(
                account: &$crate::hopper_runtime::AccountView<'_>,
                seeds: &[&[u8]],
                program_id: &$crate::hopper_runtime::Address,
...
///
/// ```ignore
/// hopper_close!(account, destination, program_id)?;
/// ```
#[macro_export]
macro_rules! hopper_close {
    ($account:expr, $destination:expr, $program_id:expr) => {
        $crate::hopper_core::account::safe_close_with_sentinel($account, $destination, $program_id)
    };
    ($account:expr, $destination:expr) => {
        $crate::hopper_core::account::safe_close_with_sentinel_unchecked($account, $destination)
    };
}
    ...
    ($name:ident; $( $field:ident : $kind:tt ; )+) => {
        ...
        impl<'a> $crate::hopper_core::accounts::HopperAccounts<'a> for $name<'a> {
            ...
            fn try_from_accounts(
                ...
                accounts: &'a [$crate::hopper_runtime::AccountView<'a>],
