...
#[inline]
pub fn require_signer(account: &AccountView<'_>) -> Result<(), ProgramError> {
    crate::check::check_signer(account)
}

/// Require that the account is writable.
#[inline]
pub fn require_writable(account: &AccountView<'_>) -> Result<(), ProgramError> {
    crate::check::check_writable(account)
}

/// Require that the account is owned by the given address.
#[inline]
pub fn require_owner(account: &AccountView<'_>, owner: &Address) -> Result<(), ProgramError> {
    crate::check::check_owner(account, owner)
}

/// Require that the account is executable (for program references).
#[inline]
pub fn require_executable(account: &AccountView<'_>) -> Result<(), ProgramError> {
