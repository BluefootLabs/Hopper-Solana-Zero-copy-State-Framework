...
//! Logs are not authenticated and may be truncated. Programs that need an
//! authenticated inner-instruction record should use Hopper's
//! `#[hopper::context(event_cpi)]` support and `ctx.emit_event_cpi(..)`, whose
//! sink verifies the program-derived event authority.
