...
Historical deployment record. A fresh finalized receipt from the exact-range
harness is still required for the current release:
...
After deploying the final source to a fresh program id, the opt-in live test
decodes the on-chain segment registry by the exact
`bids`, `asks`, and `events` ids and bounds. After one bid it verifies the
header, registry, asks, events, bids head, and every unused bids byte are
unchanged. It also verifies finalized signatures/state on public devnet and
emits a redacted JSON receipt:
