...
fn verify_program(account: &AccountView<'_>) -> ProgramResult {
    account
        .check_address(&TOKEN_PROGRAM_ID)?
        .check_executable()?;
    Ok(())
}
fn distinct(keys: &[&Address]) -> ProgramResult {
    for (i, key) in keys.iter().enumerate() {
        hopper::hopper_require!(!keys[..i].contains(key), AliasedAccount);
    }
    Ok(())
}
fn mint_decimals(account: &AccountView<'_>) -> Result<u8, ProgramError> {
