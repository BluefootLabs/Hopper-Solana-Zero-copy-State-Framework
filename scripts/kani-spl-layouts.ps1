...
if (-not $hasCargoKani) {
    ...
    if ($isNativeWindows -and $wsl) {
        ...
        if ($fullRepo -match '^([A-Za-z]):\\(.*)$') {
            ...
            wsl bash -lc "cd '$wslRepo' && sh ./scripts/kani-spl-layouts.sh$setupArg"
            exit $LASTEXITCODE
        }
    }
...
foreach ($pkg in @("hopper-token", "hopper-token-2022", "hopper-associated-token", "hopper-metaplex")) {
    Write-Host "== cargo kani -p $pkg =="
    cargo kani -p $pkg
