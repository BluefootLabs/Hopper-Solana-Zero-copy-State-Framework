...
//!
//! This is a like-for-like port of pina's `framework_verifier`
//! (`benchmarks/framework-comparison/verifier`, pinned at commit
//! `aa81c8d1d5816bb932832adce1c5550cd74b6782`): same command line, same
//! account states, same assertions, same single-run Mollusk measurement, so
//! a number produced here sits next to pina's published table on equal
//! terms. The only deltas are the Mollusk release (0.15, the workspace pin)
//! and the log-collector accessor that release exposes.
//!
//! Usage:
//!
//! ```text
//! framework-verifier --so <PATH> --program-id <BASE58> --case <hello|counter>
...
//! assumed, because the frameworks disagree: Pina, Pinocchio, Quasar and the
//! Hopper substrate store a one-byte discriminator then the bump then the
//! count; Anchor prefixes an eight-byte discriminator and pads for alignment;
//! Hopper's macro path carries its 16-byte universal header.
//!
//! Output is one JSON object on stdout. A non-zero exit means the program
//! failed functionally, which the driver treats as a measurement error rather
...
fn step(
    ...
    if ok && !expect_log.is_empty() {
        ...
        if !found {
            ok = false;
            eprintln!("framework-verifier: {name} did not log {expect_log:?}; logs:");
            for line in program_logs {
                eprintln!("  {line}");
            }
        }
...
fn fail(message: &str) -> ! {
    eprintln!("framework-verifier: {message}");
    std::process::exit(1);
}
